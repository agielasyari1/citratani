...
class Print_model extends CI_Model {
    ...
    function  __construct() {
     ...
     function p_kuitansi($id){
            ...
            $query = $this->db->get("d_penjualan_ecer")->row();
        ...
        function set_header($pdf, $id, $lembar = 0){
            ...
            $pdf->Cell(60,7,'CV. CITRA TANI',0,0,'L');
            $pdf->Cell(80,7,'',0,0,'L');
            ...
            $pdf->Cell(60,5,$this->get_nama_pembeli($id),'LRB',0,'C');
        ...
        function set_content($pdf, $id, $offset="", $limit="", $page=""){
            ...
            $query = $this->db->get("d_penjualan_ecer dpp, barang b");
            $total = 0;
            $prev_total = 0;
            if($query){
                if($page != ""){
                ...
                foreach ($query->result() as  $item){
                    ...
                    $pdf->Cell(20,5,'',0,0,'C');
                    $pdf->Cell(20,5,number_format($item->qty,0,'.',','),0,0,'R');
        ...
        function get_nama_pembeli($id){
            ...
            return $result->row(0)->nama_pembeli;
        }
        function get_prev_total($id, $i){
            
            
            $my_query = "SELECT COALESCE(SUM(harga_total),0) as total FROM ("
                    . "SELECT harga_total FROM d_penjualan_ecer WHERE no_penjualan ='".$id."' LIMIT ".$i*14 
                    . ")as temp";
