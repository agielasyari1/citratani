...
class D_PenjualanEcer extends CI_Controller {

	function  __construct() {
                parent::__construct();
                    $this->load->helper('url');
                    $this->load->model('d_penjualanecer_model', 'penjualan');
                    $this->load->model('penjualanecer_model', 'penjualanecer');
	...
	public function penjualan_baru()
	{
            ...
            if($this->session->userdata('logged_in')){
                ...
                $data['ppn'] = $ppn->ppn;
                $data['id_penjualan'] = $this->penjualanecer->generateNo();
                if(!$this->penjualanecer->cek_ada($data['id_penjualan'])){
                     $this->penjualanecer->add($data['id_penjualan']);
                }
		...
		$this->load->view('d_penjualanecer', $data);
        ...
        public function view_detail($id_penjualan)
	{
        ...
        function delete($id){
         ...
         function batal($id){
            ...
            $status = $this->penjualanecer->delete($id);
            echo json_encode(array('status' => $status));
        }
}
